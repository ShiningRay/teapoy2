  ...
  def complete
    @user.global_lock "quest#{@user.id}", 3 do
      @user.transaction do
        ...
        log = @user.quest_logs.lock.where('quest_id = ? and created_at >= ? and created_at < ?',
          self.class.name, s, e).first
...
Dir[File.join(Rails.root, 'app', 'quests', "**/*.rb")].each do |f|
