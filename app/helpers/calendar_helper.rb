...
module CalendarHelper
  ...
  #
  #   :show_special_day        => false                               # Highlights special day on the calendar using the CSS class 'special'.
  #                                                             # Defaults to true.
  ...
  def my_calendar(options = {}, &block)
    ...
    first.upto(last) do |cur|
      ...
      special_day =options[:special_day] || (Time.respond_to?(:zone) && !(zone = Time.zone).nil? ? zone.now.to_date : Date.today)
      cell_attrs[:class] += " special_day" if (cur == special_day) and options[:show_special_day]

      cal << generate_cell(cur, cell_attrs)
  ...
  def generate_cell(date, cell_attrs)
    #cell_attrs = cell_attrs.map {|k, v| %(#{k}="#{v}") }.join(" ")
    render :partial=>"archives/daylink",:locals => {:cell_attrs => cell_attrs,:date=>date}
  end
