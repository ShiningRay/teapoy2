...
<%= link_to image_tag("http://#{@group.domain}/images/qiushi/good.gif", :alt => '顶', :border=>0),
  {:only_path => false, :controller => :articles, :action => :voteup, :id => item.id, :host => @group.domain} %>
<%= item.pos_score %>
::
<%= link_to image_tag("http://#{@group.domain}/images/qiushi/bad.gif", :alt => '拍', :border=>0),
  {:only_path => false, :controller => :articles, :action => :votedown, :id => item.id, :host => @group.domain} %>
<%= item.neg_score %>
::::
